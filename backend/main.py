...
async def websocket_endpoint(websocket: WebSocket, room_id: str, userId: str = None):
    ...
    try:
        ...
        if not rooms.get(room_id):
            ...
            if room_id in room_reports: del room_reports[room_id]
        else:
            for client in rooms[room_id]:
...
async def evaluate_audio(
    ...
    try:
        ...
        # Load the audio using FastWhisper's robust internal FFmpeg decoder
        # This handles WebM flawlessly, whereas librosa crashes.
        audio_array = decode_audio(temp_file_path, sampling_rate=16000)
        sr = 16000
        
        # Execute the heavy ML feature extraction and scoring pipeline
        # If transcript_text is empty, evaluate_pipeline natively returns a 0.0 report.
        ...
        traceback.print_exc()
        report = {
            "fluency": 0.0, "lexical": 0.0, "grammar": 0.0, "pronunciation": 0.0, "overall": 0.0,
            "user_input": transcript_text if 'transcript_text' in locals() else "N/A",
            "feedback": {
                "fluency": f"Pipeline Error: {str(e)}",
                "lexical": f"Pipeline Error: {str(e)}",
                "grammar": f"Pipeline Error: {str(e)}",
                "pronunciation": f"Pipeline Error: {str(e)}"
            }
        }
