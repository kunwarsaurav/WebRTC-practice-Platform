...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="IELTS Speaking Scorer",
...
Examples:
  ...
  python main.py --audio test.wav --lang en-US --device cpu
    ...
    p.add_argument(
        "--audio",
        required=True,
        help="Path to the audio file (.wav, .mp3, .m4a, .flac)",
    )
    p.add_argument(
        "--output",
        default=None,
        help="Optional: Save JSON output to this file path (default: stdout only)",
    )
    p.add_argument(
        "--question",
        default=None,
        help="Optional: The question/topic prompt given to the speaker (for relevance checking)",
    )
    p.add_argument(
        "--lang",
        default="en-GB",
        help="LanguageTool language code for grammar checking (default: en-GB)",
    )
    p.add_argument(
        "--part",
        type=int,
        choices=[1, 2, 3],
        required=True,
        help="The part of the IELTS speaking exam (1, 2, or 3) to apply correct length penalties.",
    )
    return p
...
def evaluate_pipeline(
    ...
    """Execute the full IELTS scoring pipeline and return the result dict using provided transcription."""
    ...
    if not transcript:
        ...
        return {
            ...
            "user_input": "",
            "features": {},
            "topic_relevance": 0.0 if question else None,
            "feedback": {
                "fluency": feedback_msg,
                "lexical": feedback_msg,
                "grammar": feedback_msg,
                "pronunciation": feedback_msg,
            },
            "_meta": {
    ...
    # --- Feature extraction — all modules run in parallel ---
    ...
    def _run_pronunciation():
        ...
        return analyze_pronunciation(
            audio_path, word_timestamps, segments,
    ...
    bands = compute_bands(
        fluency_feats, lexical_feats, grammar_feats, pronunciation_feats, relevance_feats, part
    )

    # --- Feedback generation ---
    examiner_feedback = generate_feedback(
        bands, fluency_feats, lexical_feats, grammar_feats, pronunciation_feats, relevance_feats, part
    )
    ...
    result = {
        ...
        "features": {
            ...
            "grammar": grammar_feats,
            "pronunciation": pronunciation_feats,
            "relevance": relevance_feats,
        },
        "topic_relevance": relevance_feats.get("max_sentence_similarity", None) if question else None,
        "feedback": examiner_feedback,
        "_meta": {
            "audio_file": os.path.basename(audio_path),
            "grammar_lang": lang,
